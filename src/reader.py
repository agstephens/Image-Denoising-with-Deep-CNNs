...
class Reader:
   ...
   """
    ...
    def elevation(self):
        """Return the elevation of the points on the grid"""
        if type(self.__elevation) == type(None):
            self._fill_coords()
        return self.__elevation


    def sza(self):
        """Return the solar zenith angle of the points on the grid"""
        if type(self.__mu0) == type(None):
            self._read_geometry()
        #return self.__mu0
        return self.solar_zenith
    ...
    def _read_geometry(self, cdata):
        ...
        self.__mu0 = np.where(self.solar_zenith < 90.0, np.cos(self.__dtor * self.solar_zenith), 1.0)
    ...
    def _fill_coords(self):
        ...
        self.__elevation = geodetic['elevation_' + self.view][:]
